...
# Association table for Practical -> Skills
...
# Relationship mapping in Practical
Practical.skills = db.relationship('Skill', secondary=practical_skills, lazy='subquery',
        backref=db.backref('practicals', lazy=True))
...
def get_cohort_title(end_year):
    # Dynamic calculation based on current year (Assumed 2026 for context)
    current_year = datetime.utcnow().year
    current_month = datetime.utcnow().month
    # If we are before September, the academic year ends this year.
    academic_end_year = current_year if current_month < 9 else current_year + 1
    
    if end_year == academic_end_year: return "Year 13"
    elif end_year == academic_end_year + 1: return "Year 12"
    elif end_year < academic_end_year:
        diff = academic_end_year - end_year
...
@login_required
def add_cohort():
    start = int(request.form.get('start_year'))
    end = int(request.form.get('end_year'))
    db.session.add(Cohort(start_year=start, end_year=end))
    db.session.commit()
...
def cohort_view(id):
    ...
    # Build matrix mapping for headers
    matrix = []
    for p in practicals:
        matrix.append({'id': p.id, 'name': p.name, 'skills': [s.name for s in p.skills]})

    students_data = []
    for s in cohort.students:
        assessments = Assessment.query.filter_by(student_id=s.id).all()
        
        # Track ticks per skill across all practicals
        skill_totals = {sk.name: 0 for sk in all_skills}
        prac_totals = {p.id: 0 for p in practicals}
        
        # Track specific assessment cell states {prac_id: {skill_name: True}}
        ...
        # Fail condition: strictly < 3 ticks for ANY of the 11 skills
        status = "Pass"
        for count in skill_totals.values():
            if count < 3:
                status = "Fail"
                break
        ...
        students_data.append({
            ...
            'prac_totals': prac_totals,
            'assessed_cells': assessed_cells
        })

    # Used for hover tooltips mappings
    prac_names = {p.id: p.name for p in practicals}

    return render_template('cohort.html', 
                           cohort=cohort, 
                           title=get_cohort_title(cohort.end_year),
                           matrix=json.dumps(matrix),
                           students_json=json.dumps(students_data),
                           prac_names=json.dumps(prac_names))
...
def bulk_add(id):
    ...
    for line in data:
        ...
        if format_type == "First Last":
            parts = line.split()
            if len(parts) >= 2: fname, lname = parts[0], " ".join(parts[1:])
    ...
    db.session.commit()
    flash(f'Bulk students added.', 'success')
    return redirect(url_for('cohort_view', id=id))
...
def student_view(id):
    ...
    prac_scores = {p: 0 for p in practicals}
    skill_scores = {sk: 0 for sk in all_skills}
    total_score = len(assessments)
    
    for a in assessments:
        p = Practical.query.get(a.practical_id)
        sk = Skill.query.get(a.skill_id)
        prac_scores[p] += 1
        skill_scores[sk] += 1
        
    current_date = datetime.utcnow().strftime('%d/%m/%Y')
    
    return render_template('student.html', student=student, prac_scores=prac_scores, skill_scores=skill_scores, total_score=total_score, date=current_date)
...
def grade(student_id, prac_id):
    ...
    skills = practical.skills # Only the specific skills for this practical
    ...
    # Bundle teacher names for signatures
    signatures = {}
    for a in assessments:
        t = Teacher.query.get(a.teacher_id)
        signatures[a.skill_id] = f"{t.first_name[0]} {t.last_name} ({a.date_signed.strftime('%d/%m/%Y')})"
